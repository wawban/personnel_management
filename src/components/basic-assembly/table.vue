<template>
  <div class="zebra-table">
    <PageHeader :title="pageheadertitle" :class="{'zebra-table-titleTop': true, 'zebra-table-top': !option.search == false}">
      <!-- <div style="height:20px" v-if="pageheadertitle"></div> -->
      <slot name="headerTop"></slot>
      <zebra-form v-if="!option.search == false && searchOption.column && searchOption.column.length > 0" :formData="searchFormData" :defalutFormData="searchFormData" class="search-form" :option="searchOption" @submit="searchHandle" :isSearch="true">
      </zebra-form>
      <div class="table-top">
    ...
    <el-dialog
      ...
      <zebra-form :formData="rowData" :defalutFormData="rowData" :option="formOpton" @submit="submitHandle" :isSearch="false" @cancalClick="handleClose">
      </zebra-form>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    styleHeight () {
      ...
      if($('.zebra-table-top') && $('.zebra-table-top').length > 0) {
        if($('.pageheader-top')) {
          height -= $('.pageheader-top').height() - 10
        }
        if($('.zebra-table-top') && $('.zebra-table-top').length > 0) {
          height -= $('.zebra-table-top').height() - 20
        }else{
          height -= 8
        }
      }else{
        if($('.zebra-table-titleTop') && $('.zebra-table-titleTop').length > 0) {
          height -= ($('.zebra-table-titleTop').height() - 18)
        }else{
          height -= 8
        }
      }
