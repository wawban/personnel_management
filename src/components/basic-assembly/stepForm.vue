<template>
  <zebra-tab :active="activeName" type="card" :option="stepFormOption" @tab-click="tabClick">
    <template v-for="(item, index) in stepFormOption.column" :slot="item.name">
      <zebra-form v-if="item.formOption" :key="item.name+'level'" :defalutFormData="item.defaultData" :option="item.formOption" @submit="submitHandle">
        <template slot="formButton">
          <zebra-button v-if="index > 0" @click="lastStep(index)">上一步</zebra-button>
        </template>
      </zebra-form>
    </template>
  </zebra-tab>
