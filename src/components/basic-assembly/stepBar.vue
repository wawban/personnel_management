    ...
    <div class="step-bar-content">
      ...
      >
        ...
        <div class="step-bar-button" :style="'display:flex;justify-content:'+ (formItem.toolAlign ? formItem.toolAlign : 'center') +';'">
          <zebra-button size="mini" type="primary" v-for="(bi, bindex) in formItem.dicData[index].btns" :key="bi.name+bindex" @click="btnClick(bi)">{{bi.name}}</zebra-button>
        </div>
      </div>
    </div>
  </div>
