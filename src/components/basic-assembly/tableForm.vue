    ...
    >
      <template slot="menu" slot-scope="scope">
        <div style="width: 100%;display: flex;justify-content: center;">
          <zebra-button v-if="!item.editable && item.editBtn" size="mini" type="text" @click="showForm(item, 'edit', scope.row, scope.index)">编辑</zebra-button>
          <zebra-button v-if="!item.editable && item.viewBtn" size="mini" type="text" @click="showForm(item, 'view', scope.row, scope.index)">详情</zebra-button>
          <slot name="menuBtn" :row="scope.row" :index="scope.index"></slot>
          <zebra-button v-if="!item.editable && item.delBtn" size="mini" type="text" @click="delRowHandle(scope.row, scope.index)">删除</zebra-button>
        </div>
      </template>
      <template slot="menuLeft">
        <zebra-button v-if="!item.editable && item.addBtn" size="mini" type="primary" @click="showForm(item, 'add')">新增</zebra-button>
      </template>
