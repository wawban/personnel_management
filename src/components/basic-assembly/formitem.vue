<template>
  <div class="zebra-form-item" style="display:flex;align-items:center;">
    <el-input
    ...
    <div v-if="item.type == 'tableForm'" style="flex:1;width:100%;">
      <tableForm :item="item" :option="tableFormOption"
        ...
        <template slot="menuBtn" slot-scope="scope">
          <zebra-button v-if="item.editable && item.delBtn" type="text" @click="deleteRow(scope.row, scope.index)">删除</zebra-button>
        </template>
      </tableForm>
      <el-row style="margin-top:10px;" v-if="item.editable && item.addBtn">
        <zebra-button size="mini" @click="addRowHandle">新增</zebra-button>
      </el-row>
    </div>
    ...
    <!-- 按钮 -->
    <zebra-button v-if="item.type === 'button'" :disabled="item.disabled" :type="item.buttonType" :round="item.buttonRound" :size="item.size" @click="btnClick">{{item.text}}</zebra-button>
  ...
  methods: {
    ...
    initEditor (prop) {
      ...
      this.$nextTick(() => {
        ...
        _this.editor.config.uploadImgServer = '/api/file/upload/zebrahtmleditor'
