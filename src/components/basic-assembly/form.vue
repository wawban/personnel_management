<template>
  <el-form
    ...
    :class="{'zebra-form': true, 'zebra-form-autoflexable': (option.labelWidth == 'auto' || option.labelwidth == 'auto' || option.formAuto )}"
    ...
    resetForm (formName) {
      if (this.option.isSearch === true) {
        ...
        if(this.resetData.startsWith('{')) {
          let obj = JSON.parse(this.resetData)
          for(let i in obj) {
            this.$set(this.formDatas, i, obj[i])
          }
