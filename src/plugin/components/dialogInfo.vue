<template>
  ...
  <div class="form-show-info-dialog">
    <el-dialog
      ...
      :fullscreen="(formType == 'normalForm' || formType == 'detailForm') ? formOption.fullscreen: true"
    ...
    >
      <div v-if="data.viewJson">
        <zebra-form v-if="formType == 'normalForm' || formType == 'detailForm'" :option="formOption" :formData="formData" @submit="formSubmit">
          <!-- 自定义按钮 -->
          <template slot="formButton">
            <zebra-button size="mini" v-for="(item, index) in formOption.btnSetting" :key="item.name+'slotbtn'+index" @click="slotbtnClickHandle(item, index)">{{item.name}}</zebra-button>
          </template>
        </zebra-form>
        <zebra-form-level v-if="formType == 'multiLevelForm'" :option="formOption" :formData="formData" @submit="formSubmit">
        </zebra-form-level>
        <!-- 流程表单 -->
        <div v-if="formType == 'flowable'">
          <zebra-form :option="basicOption" @submit="formSubmit" :formData="formData.basic" @formChange="formChange">
            <!-- 自定义按钮 -->
            <template slot="formButton">
              <zebra-button size="mini" v-for="(item, index) in basicOption.btnSetting" :key="item.name+'slotbtn'+index" @click="slotbtnClickHandle(item, index)">{{item.name}}</zebra-button>
            </template>
          </zebra-form>
          <zebra-form :option="formOption" @submit="formSubmit" :formData="formData.form" @formChange="formChange">
            <!-- 自定义按钮 -->
            <template slot="formButton">
              <zebra-button size="mini" v-for="(item, index) in formOption.btnSetting" :key="item.name+'slotbtn'+index" @click="slotbtnClickHandle(item, index)">{{item.name}}</zebra-button>
            </template>
    ...
    <el-dialog
      ...
      <el-row style="display:flex;justify-content:center;align-items:center;margin-top:20px;">
        <zebra-button size="mini" type="primary" @click="assSubmit">提交</zebra-button>
